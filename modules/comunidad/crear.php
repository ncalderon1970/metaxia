...
function cm_old(array $old, string $key, string $default = ''): string
{
...
$old = $_SESSION['comunidad_form_old'] ?? [];
$errorFormulario = (string)($_SESSION['comunidad_form_error'] ?? '');
...
<section class="cm-panel">
    ...
    <div class="cm-panel-body">
        ...
        <div class="cm-note">
            ...
            Si el RUN ya existe, el sistema conservará los datos ingresados para que puedas corregirlo.
        </div>

        <form method="post" action="<?= APP_URL ?>/modules/comunidad/guardar.php" class="cm-form" autocomplete="off">
            <?= CSRF::field() ?>
            ...
            <div class="cm-field">
                ...
                <input class="cm-control" type="text" name="run" value="<?= e(cm_old($old, 'run')) ?>" placeholder="11111111-1" required autofocus>
            </div>

            <div class="cm-field">
                <label class="cm-label">Nombres *</label>
                <input class="cm-control" type="text" name="nombres" value="<?= e(cm_old($old, 'nombres')) ?>" placeholder="Nombres" required>
            </div>

            <div class="cm-field">
                <label class="cm-label">Apellido paterno</label>
                <input class="cm-control" type="text" name="apellido_paterno" value="<?= e(cm_old($old, 'apellido_paterno')) ?>" placeholder="Apellido paterno">
            </div>

            <div class="cm-field">
                <label class="cm-label">Apellido materno</label>
                <input class="cm-control" type="text" name="apellido_materno" value="<?= e(cm_old($old, 'apellido_materno')) ?>" placeholder="Apellido materno">
            </div>

            <?php if ($tipo === 'alumnos'): ?>
                <div class="cm-field">
                    <label class="cm-label">Curso</label>
                    <input class="cm-control" type="text" name="curso" value="<?= e(cm_old($old, 'curso')) ?>" placeholder="7° Básico A">
                </div>

                <div class="cm-field">
                    <label class="cm-label">Fecha de nacimiento</label>
                    <input class="cm-control" type="date" name="fecha_nacimiento" value="<?= e(cm_old($old, 'fecha_nacimiento')) ?>">
                </div>
            <?php endif; ?>
            ...
            <?php if ($tipo === 'apoderados'): ?>
                <div class="cm-field">
                    ...
                    <input class="cm-control" type="text" name="parentesco" value="<?= e(cm_old($old, 'parentesco')) ?>" placeholder="Madre, padre, tutor, otro">
                </div>
            <?php endif; ?>
            ...
            <?php if ($tipo === 'docentes'): ?>
                <div class="cm-field">
                    ...
                    <input class="cm-control" type="text" name="especialidad" value="<?= e(cm_old($old, 'especialidad')) ?>" placeholder="Lenguaje, Matemática, PIE, etc.">
                </div>
            <?php endif; ?>
            ...
            <?php if ($tipo === 'asistentes'): ?>
                <div class="cm-field">
                    ...
                    <input class="cm-control" type="text" name="cargo" value="<?= e(cm_old($old, 'cargo')) ?>" placeholder="Inspectoría, convivencia, administración, etc.">
                </div>
            <?php endif; ?>

            <div class="cm-field">
                <label class="cm-label">Email</label>
                <input class="cm-control" type="email" name="email" value="<?= e(cm_old($old, 'email')) ?>" placeholder="correo@dominio.cl">
            </div>

            <div class="cm-field">
                <label class="cm-label">Teléfono</label>
                <input class="cm-control" type="text" name="telefono" value="<?= e(cm_old($old, 'telefono')) ?>" placeholder="+56912345678">
            </div>

            <div class="cm-field full">
                <label class="cm-label">Dirección</label>
                <input class="cm-control" type="text" name="direccion" value="<?= e(cm_old($old, 'direccion')) ?>" placeholder="Dirección">
            </div>

            <div class="cm-field">
                <label class="cm-label">Estado</label>
                <select class="cm-control" name="activo">
                    <option value="1" <?= cm_old($old, 'activo', '1') === '1' ? 'selected' : '' ?>>Activo</option>
                    <option value="0" <?= cm_old($old, 'activo', '1') === '0' ? 'selected' : '' ?>>Inactivo</option>
                </select>
            </div>
