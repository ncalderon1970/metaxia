...
function fin_export_csv(array $rows): void
{
    ...
    foreach ($rows as $row) {
        ...
        fputcsv($out, [
            ...
            $riesgoTexto,
            $riesgoDetalle,
            (string)($row['contacto_comercial'] ?? ''),
            (string)($row['email_comercial'] ?? ''),
            (string)($row['telefono_comercial'] ?? ''),
        ], ';');
    }
...
try {
    $stmtPlanes = $pdo->query("
        SELECT
            COALESCE(NULLIF(plan, ''), 'sin_plan') AS plan_codigo,
            COALESCE(NULLIF(plan, ''), 'Sin plan') AS plan_nombre,
            COUNT(*) AS total,
        ...
        FROM colegios
        GROUP BY COALESCE(NULLIF(plan, ''), 'sin_plan'), COALESCE(NULLIF(plan, ''), 'Sin plan')
                        ...
                        <article class="fin-card">
                            ...
                            <div style="margin-top:.7rem;">
                                <span class="fin-badge blue">
                                    <?= e(fin_pick($colegio, 'plan', 'Sin plan')) ?>
                                </span>
                            ...
                            <div class="fin-meta">
                                Vencimiento: <?= e(fin_fecha($colegio['fecha_vencimiento'] ?? null)) ?>
                            </div>

                            <div class="fin-meta">
                                Contacto: <?= e(fin_pick($colegio, 'contacto_comercial')) ?> ·
                                <?= e(fin_pick($colegio, 'email_comercial')) ?> ·
                                <?= e(fin_pick($colegio, 'telefono_comercial')) ?>
                            </div>
