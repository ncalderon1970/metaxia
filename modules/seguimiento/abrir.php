...
function seg_abrir_badge(string $value): string
{
    return match (strtolower($value)) {
        ...
        'amarillo', 'media', 'seguimiento', 'investigacion', 'revision_inicial' => 'warn',
...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($accion === 'actualizar_control') {
            ...
            $stmt->execute([
                $casoId,
                'Se actualizó estado, semáforo, prioridad o reanálisis del caso.',
                $userId ?: null,
            ]);
...
$stmtContadores = $pdo->prepare("
    SELECT
        (SELECT COUNT(*) FROM caso_participantes WHERE caso_id = ?) AS participantes,
        (SELECT COUNT(*) FROM caso_declaraciones WHERE caso_id = ?) AS declaraciones,
        (SELECT COUNT(*) FROM caso_evidencias WHERE caso_id = ?) AS evidencias,
...
");
$stmtContadores->execute([$casoId, $casoId, $casoId, $casoId, $casoId]);
    ...
    <aside>
        <div class="seg-card">
            ...
            <div class="seg-data">
                ...
                <span>
                    <span class="seg-badge <?= e(seg_abrir_badge((string)$caso['semaforo'])) ?>">
                        <?= e(seg_abrir_label($caso['semaforo'])) ?>
                    </span>
                </span>
            ...
            <form method="post">
                ...
                <div style="margin-top:1rem;" class="seg-grid-2">
                    <div>
                        <label class="seg-label">Semáforo</label>
                        <select class="seg-control" name="semaforo">
