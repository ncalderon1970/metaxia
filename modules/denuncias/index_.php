...
$semaforoFiltro = clean((string)($_GET['semaforo'] ?? ''));
...
try {
    ...
    ");
    $stmt->execute([$colegioId]);
    $totalRojo = (int)$stmt->fetchColumn();
...
<section class="den-kpis">
    ...
    <div class="den-kpi">
        ...
        <strong><?= number_format($totalRojo, 0, ',', '.') ?></strong>
    </div>
...
<section class="den-panel">
    ...
    <div class="den-panel-body">
        <form method="get" class="den-filter">
            ...
            <div>
                <label class="den-label">Semáforo</label>
                <select class="den-control" name="semaforo">
                    <option value="">Todos</option>
                    <option value="verde" <?= $semaforoFiltro === 'verde' ? 'selected' : '' ?>>Verde</option>
                    <option value="amarillo" <?= $semaforoFiltro === 'amarillo' ? 'selected' : '' ?>>Amarillo</option>
                    <option value="rojo" <?= $semaforoFiltro === 'rojo' ? 'selected' : '' ?>>Rojo</option>
                </select>
            </div>
            ...
            <table class="den-table">
                ...
                <tbody>
                    <?php foreach ($casos as $caso): ?>
                        ...
                        $semaforo = strtolower((string)$caso['semaforo']);
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge-den <?= e(den_badge_class($semaforo)) ?>">
                                    <?= e(den_label($semaforo)) ?>
                                </span>
                            </td>
                            ...
                            <td>
                                <?php if ((string)$caso['estado'] === 'borrador'): ?>
                                    <a class="den-action" href="<?= APP_URL ?>/modules/denuncias/editar_borrador.php?id=<?= (int)$caso['id'] ?>"
