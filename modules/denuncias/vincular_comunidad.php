...
function vc_sql_uno(string $tipo): ?string
{
    return match ($tipo) {
        'alumnos' => "
            SELECT
...
function vc_sql_busqueda(string $tipo): ?string
{
    return match ($tipo) {
        'alumnos' => "
            SELECT
            ...
            WHERE colegio_id = ?
              ...
              AND (
                 ...
                 OR CONVERT(COALESCE(telefono, '') USING utf8mb4) COLLATE utf8mb4_unicode_ci LIKE ?
                 OR CONVERT(COALESCE(curso, '') USING utf8mb4) COLLATE utf8mb4_unicode_ci LIKE ?
                 OR REPLACE(REPLACE(REPLACE(COALESCE(run, ''), '.', ''), '-', ''), ' ', '') LIKE ?
              )
            ORDER BY apellido_paterno ASC, apellido_materno ASC, nombres ASC, id DESC
            LIMIT 80
        ",
        'apoderados' => "
            SELECT
            ...
            WHERE colegio_id = ?
              ...
              AND (
                    CONVERT(COALESCE(run, '') USING utf8mb4) COLLATE utf8mb4_unicode_ci LIKE ?
                 OR CONVERT(CONCAT_WS(' ', nombres, apellido_paterno, apellido_materno, nombre) USING utf8mb4) COLLATE utf8mb4_unicode_ci LIKE ?
                 OR CONVERT(COALESCE(email, '') USING utf8mb4) COLLATE utf8mb4_unicode_ci LIKE ?
                 OR CONVERT(COALESCE(telefono, '') USING utf8mb4) COLLATE utf8mb4_unicode_ci LIKE ?
                 OR REPLACE(REPLACE(REPLACE(COALESCE(run, ''), '.', ''), '-', ''), ' ', '') LIKE ?
              )
            ORDER BY apellido_paterno ASC, apellido_materno ASC, nombres ASC, nombre ASC, id DESC
            LIMIT 80
        ",
        'docentes' => "
            SELECT
...
function vc_obtener_persona(PDO $pdo, string $tipo, int $personaId, int $colegioId): ?array
{
    $sql = vc_sql_uno($tipo);
    if ($sql === null || $personaId <= 0 || $colegioId <= 0) {
        return null;
    }
...
function vc_buscar_personas(PDO $pdo, string $tipo, int $colegioId, string $q): array
{
    $sql = vc_sql_busqueda($tipo);
    if ($sql === null || $colegioId <= 0 || trim($q) === '') {
        return [];
    }

    $like = '%' . $q . '%';
    $runLimpio = preg_replace('/[^0-9Kk]/', '', $q) ?: $q;
    $runLike = '%' . $runLimpio . '%';

    $params = match ($tipo) {
        'alumnos' => [$colegioId, $like, $like, $like, $like, $like, $runLike],
        'apoderados' => [$colegioId, $like, $like, $like, $like, $runLike],
        'docentes', 'asistentes' => [$colegioId, $like, $like, $like, $like, $like, $runLike],
        default => [],
    };
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $persona = vc_obtener_persona($pdo, $tipo, $personaId, $colegioId);

        if (!$persona) {
            throw new RuntimeException('La persona seleccionada no existe, no está activa o no pertenece al establecimiento.');
        }

        $tipoPersona = (string)$tipos[$tipo]['tipo_persona'];
        $nombreReferencial = mb_strtoupper(vc_nombre_completo($persona), 'UTF-8');
        $run = cleanRun(vc_pick($persona, ['run'], '0-0'));
        ...
        $stmtExiste = $pdo->prepare("
            ...
            WHERE cp.caso_id = ?
              ...
              AND c.colegio_id = ?
        ");
        $stmtExiste->execute([$casoId, $tipoPersona, $personaId, $colegioId]);
        ...
        $stmtInsert = $pdo->prepare("
            INSERT INTO caso_participantes (
                ...
                ?, ?, ?, ?, ?, 1, NOW(), ?, ?, 0, NULL, ?, 'Vinculado desde comunidad educativa.', NOW()
            )
        ");
        $stmtInsert->execute([
            $casoId,
        ...
        try {
            $stmtHistorial = $pdo->prepare("
                INSERT INTO caso_historial (
                    ...
                    ?, 'participante', 'Participante vinculado desde comunidad educativa', ?, ?, NOW()
                )
            ");
            $stmtHistorial->execute([
                $casoId,
                $nombreReferencial . ' fue vinculado como ' . vc_label($rolEnCaso) . '.',
                $userId > 0 ? $userId : null,
            ]);
        } catch (Throwable $e) {
            // El historial no debe impedir la vinculación principal.
        }
    ...
    if ($q !== '') {
        $resultados = vc_buscar_personas($pdo, $tipo, $colegioId, $q);
    }
    ...
    $stmtPart = $pdo->prepare("
        ...
        ORDER BY cp.id DESC
    ");
    $stmtPart->execute([$casoId, $colegioId]);
...
<section class="vc-hero">
    ...
    <p>
        ...
        víctima, denunciante, denunciado, testigo o otro interviniente.
    </p>
