...
// ══════════════════════════════════════════════════════════════
// 4. APODERADOS de alumnos participantes
// ══════════════════════════════════════════════════════════════
$apoderadosPorAlumno = [];
$alumnoIds = array_filter(array_map(fn($p) =>
    ($p['tipo_persona'] === 'alumno') ? (int)($p['persona_id'] ?? 0) : 0,
    $participantes
));
if ($alumnoIds) {
    $in = implode(',', array_fill(0, count($alumnoIds), '?'));
    $stmtAp = $pdo->prepare("
        SELECT aa.alumno_id, aa.es_titular, aa.parentesco, aa.tipo_relacion,
               aa.puede_retirar, aa.vive_con_estudiante,
               ap.nombre, ap.run, ap.telefono, ap.telefono_secundario,
               ap.email, ap.direccion
        FROM alumno_apoderado aa
        INNER JOIN apoderados ap ON ap.id = aa.apoderado_id
        WHERE aa.alumno_id IN ($in) AND aa.activo = 1
        ORDER BY aa.alumno_id ASC, aa.es_titular DESC
    ");
    $stmtAp->execute(array_values($alumnoIds));
    foreach ($stmtAp->fetchAll() as $apo) {
        $apoderadosPorAlumno[(int)$apo['alumno_id']][] = $apo;
    }
}
...
$marcadoresRex782 = [];
if ($clasif) {
    $marcadores21809  = json_decode((string)($clasif['ley21809_flags']  ?? '[]'), true) ?: [];
    $marcadoresRex782 = json_decode((string)($clasif['rex782_flags']    ?? '[]'), true) ?: [];
}
...
<head>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .inf-flag {
        ...
        font-size: .79rem; padding: .3rem .5rem;
        border-radius: 5px; border: 1px solid #e2e8f0; background: #f8fafc;
    }
    .inf-flag.activo { background: #eff6ff; border-color: #bfdbfe; color: #1e3a8a; }
    .inf-flag.activo .inf-flag-dot { color: #2563eb; }
    .inf-flag-dot { font-size: .9rem; flex-shrink: 0; color: #cbd5e1; }
...
<div class="inf-wrap">
    ...
    <div class="inf-indice">
        ...
        <a href="#sec5">5. Marcadores normativos</a>
        <a href="#sec6">6. Evaluación de riesgo</a>
        <a href="#sec7">7. Plan de acción</a>
        <a href="#sec8">8. Seguimiento por participante</a>
        <a href="#sec9">9. Comunicaciones al apoderado</a>
        <a href="#sec10">10. Gestión ejecutiva</a>
        <a href="#sec11">11. Declaraciones</a>
        <a href="#sec12">12. Evidencias</a>
        <?php if ($aulaSegura): ?><a href="#sec13">13. Aula Segura</a><?php endif; ?>
    ...
    <div class="inf-body">
        ...
        <div class="inf-section" id="sec1">
            ...
            <div class="inf-section-body">
                <table class="inf-table">
                    ...
                    <tr><th>Marco legal aplicable</th>
                        <td><?= e(inf_label((string)($caso['marco_legal'] ?? 'ley21809'))) ?></td></tr>
        ...
        <div class="inf-section" id="sec3">
            <div class="inf-section-head">
                ...
                <div class="inf-section-title">Participantes (<?= count($participantes) ?>)</div>
            </div>
            <div class="inf-section-body">
                ...
                <?php foreach ($participantes as $p):
                    ...
                    $apos = ($p['tipo_persona'] === 'alumno' && $alumnoId)
                        ? ($apoderadosPorAlumno[$alumnoId] ?? []) : [];
                ?>
                <div class="inf-part-card">
                    <div class="inf-part-head">
                        <span class="inf-rol-badge <?= $rolCss ?>"><?= e(inf_label($rol)) ?></span>
                    ...
                    <div class="inf-part-body">
                        ...
                        <div style="margin-top:.6rem;">
                            ...
                            <table class="inf-table" style="font-size:.78rem;margin-bottom:.4rem;">
                                <tr><th>Nombre</th>
                                    <td><?= e((string)($apo['nombre'] ?? '—')) ?>
                                        <?php if ($apo['es_titular']): ?>
                                            <span style="background:#1a3a5c;color:#fff;border-radius:4px;
                                                         font-size:.65rem;padding:.1rem .35rem;margin-left:.3rem;">Titular</span>
                                        <?php endif; ?>
                                    </td></tr>
                                <tr><th>RUN</th><td><?= e((string)($apo['run'] ?? '—')) ?></td></tr>
                                <tr><th>Parentesco</th>
                                    <td><?= e(inf_label((string)($apo['parentesco'] ?? $apo['tipo_relacion'] ?? '—'))) ?></td></tr>
                                <?php if (!empty($apo['telefono'])): ?>
                                <tr><th>Teléfono</th>
                                    <td><?= e((string)$apo['telefono']) ?>
                                        <?= !empty($apo['telefono_secundario']) ? ' / ' . e((string)$apo['telefono_secundario']) : '' ?></td></tr>
                                <?php endif; ?>
                                <?php if (!empty($apo['email'])): ?>
                                <tr><th>Correo</th><td><?= e((string)$apo['email']) ?></td></tr>
                                <?php endif; ?>
                                <tr><th>Puede retirar</th><td><?= inf_si_no($apo['puede_retirar']) ?></td></tr>
                                <tr><th>Vive con estudiante</th><td><?= inf_si_no($apo['vive_con_estudiante']) ?></td></tr>
                            </table>
                            <?php endforeach; ?>
                        </div>
                        <?php endif; ?>
        ...
        <div class="inf-section" id="sec4">
            ...
            <div class="inf-section-body">
                ...
                <?php else: ?>
                <table class="inf-table">
                    <?php if (!empty($clasif['area_mineduc'])): ?>
                    <tr><th>Área MINEDUC</th><td><?= e(inf_label((string)$clasif['area_mineduc'])) ?></td></tr>
                    <?php endif; ?>
                    <?php if (!empty($clasif['ambito_mineduc'])): ?>
                    <tr><th>Ámbito</th><td><?= e(inf_label((string)$clasif['ambito_mineduc'])) ?></td></tr>
                    <?php endif; ?>
                    <?php if (!empty($clasif['tipo_conducta'])): ?>
                    <tr><th>Tipo de conducta</th><td><?= e(inf_label((string)$clasif['tipo_conducta'])) ?></td></tr>
                    <?php endif; ?>
                    <?php if (!empty($clasif['conducta_principal'])): ?>
                    <tr><th>Conducta principal</th><td><?= e(inf_label((string)$clasif['conducta_principal'])) ?></td></tr>
                    <?php endif; ?>
                    <tr><th>Gravedad</th><td><?= e(inf_label((string)($clasif['gravedad'] ?? 'media'))) ?></td></tr>
                    <tr><th>Reiteración</th><td><?= inf_si_no($clasif['reiteracion'] ?? 0) ?></td></tr>
                    <tr><th>Involucra adulto</th><td><?= inf_si_no($clasif['involucra_adulto'] ?? 0) ?></td></tr>
                    <tr><th>Acoso escolar</th><td><?= inf_si_no($clasif['acoso_escolar'] ?? 0) ?></td></tr>
                    <tr><th>Violencia física</th><td><?= inf_si_no($clasif['violencia_fisica'] ?? 0) ?></td></tr>
                    <tr><th>Violencia psicológica</th><td><?= inf_si_no($clasif['violencia_psicologica'] ?? 0) ?></td></tr>
                    <tr><th>Violencia sexual</th><td><?= inf_si_no($clasif['violencia_sexual'] ?? 0) ?></td></tr>
                    <tr><th>Discriminación</th><td><?= inf_si_no($clasif['discriminacion'] ?? 0) ?></td></tr>
                    <tr><th>Ciberacoso</th><td><?= inf_si_no($clasif['ciberacoso'] ?? 0) ?></td></tr>
                    <tr><th>Posible Aula Segura</th><td><?= inf_si_no($clasif['posible_aula_segura'] ?? 0) ?></td></tr>
                    <tr><th>Requiere denuncia/derivación</th><td><?= inf_si_no($clasif['requiere_denuncia'] ?? 0) ?></td></tr>
                    <?php if (!empty($clasif['entidad_derivacion'])): ?>
                    <tr><th>Entidad de derivación</th><td><?= e((string)$clasif['entidad_derivacion']) ?></td></tr>
                    <?php endif; ?>
                    <?php if (!empty($clasif['categoria_convivencia'])): ?>
                    <tr><th>Categoría convivencia</th><td><?= e(inf_label((string)$clasif['categoria_convivencia'])) ?></td></tr>
                    <?php endif; ?>
        ...
        <!-- ══════════════════════════════════════════════════
             SECCIÓN 5 · MARCADORES NORMATIVOS (Ley 21.809 / REX 782)
        ═══════════════════════════════════════════════════ -->
        <div class="inf-section" id="sec5">
            <div class="inf-section-head">
                <div class="inf-section-num">5</div>
                <div class="inf-section-title">Marcadores normativos</div>
            </div>
            <div class="inf-section-body">
                <?php
                $flags21809def = [
                    'afecta_buen_trato'            => 'Afecta buen trato',
                    'acoso_escolar'                => 'Posible acoso escolar',
                    'violencia_fisica'             => 'Violencia física',
                    'violencia_psicologica'        => 'Violencia psicológica',
                    'discriminacion'               => 'Discriminación',
                    'ciberacoso_medios_digitales'  => 'Ciberacoso / medio digital',
                    'afecta_salud_mental'          => 'Afecta bienestar o salud mental',
                    'requiere_derivacion'          => 'Requiere derivación o apoyo externo',
                ];
                $flagsRex782def = [
                    'requiere_medida_formativa'    => 'Medida formativa',
                    'requiere_apoyo_psicosocial'   => 'Apoyo psicosocial',
                    'requiere_medida_reparatoria'  => 'Medida reparatoria',
                    'posible_medida_disciplinaria' => 'Posible medida disciplinaria',
                    'requiere_justo_procedimiento' => 'Justo procedimiento',
                    'requiere_proporcionalidad'    => 'Control de proporcionalidad',
                    'gestion_colaborativa_conflicto'=> 'Gestión colaborativa del conflicto',
                    'posible_expulsion_cancelacion'=> 'Medida gravosa en evaluación',
                ];
                ?>

                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
                    <div>
                        <div style="font-size:.74rem;font-weight:700;color:#1a3a5c;
                                    text-transform:uppercase;letter-spacing:.06em;margin-bottom:.5rem;">
                            🛡 Ley 21.809 — Convivencia y buen trato
                        </div>
                        <div class="inf-flags-grid" style="grid-template-columns:1fr;">
                            <?php foreach ($flags21809def as $k => $label):
                                $activo = in_array($k, $marcadores21809, true);
                            ?>
                            <div class="inf-flag <?= $activo ? 'activo' : '' ?>">
                                <span class="inf-flag-dot"><?= $activo ? '●' : '○' ?></span>
                                <?= e($label) ?>
                                <?php if (!$activo): ?>
                                    <span style="color:#cbd5e1;margin-left:auto;font-size:.72rem;">No marcado</span>
                                <?php endif; ?>
                            </div>
                            <?php endforeach; ?>
                        </div>
                    ...
                    <div>
                        <div style="font-size:.74rem;font-weight:700;color:#1a3a5c;
                                    text-transform:uppercase;letter-spacing:.06em;margin-bottom:.5rem;">
                            📋 REX 782 — Medidas y procedimiento
                        </div>
                        <div class="inf-flags-grid" style="grid-template-columns:1fr;">
                            <?php foreach ($flagsRex782def as $k => $label):
                                $activo = in_array($k, $marcadoresRex782, true);
                            ?>
                            <div class="inf-flag <?= $activo ? 'activo' : '' ?>">
                                <span class="inf-flag-dot"><?= $activo ? '●' : '○' ?></span>
                                <?= e($label) ?>
                ...
                <?php if (!$clasif): ?>
                    <p class="inf-muted" style="margin-top:.5rem;">Sin clasificación normativa registrada — marcadores no disponibles.</p>
                <?php endif; ?>
            </div>
        </div>

        <!-- ══════════════════════════════════════════════════
             SECCIÓN 6 · PAUTA DE EVALUACIÓN DE RIESGO
        ═══════════════════════════════════════════════════ -->
        ...
        <!-- ══════════════════════════════════════════════════
             SECCIÓN 8 · BITÁCORA DE SEGUIMIENTO POR PARTICIPANTE
        ═══════════════════════════════════════════════════ -->
        ...
        <!-- ══════════════════════════════════════════════════
             SECCIÓN 9 · COMUNICACIONES AL APODERADO
        ═══════════════════════════════════════════════════ -->
        ...
        <!-- ══════════════════════════════════════════════════
             SECCIÓN 13 · AULA SEGURA (condicional)
        ═══════════════════════════════════════════════════ -->
        <?php if ($aulaSegura): ?>
        <div class="inf-section" id="sec13">
            <div class="inf-section-head">
                <div class="inf-section-num">13</div>
